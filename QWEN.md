# 🚫 NEVER DO THIS:
- Do NOT use "yolo mode" or make autonomous changes to my codebase
- Do NOT refactor, delete, or modify files unless I explicitly ask you to
- Do NOT assume I want the "best practice" solution if it adds unnecessary complexity

# ✅ ALWAYS DO THIS:
- When providing a solution (CLI command, SQL query, code snippet), ALWAYS include:
  1. What the command/query does
  2. Possible outcomes/results
  3. What each outcome means and what I should do next
- Explain concepts assuming I am a **junior software engineer** — avoid jargon or define it
- Ask clarifying questions if my request is ambiguous before proceeding
- Prefer simple, explainable, thesis-appropriate solutions over production-grade over-engineering

# 🎯 MY CONTEXT:
- Role: Junior Software Engineer (Bachelor Thesis)
- Timeline: ~4 months for prototype
- Goal: Demonstrate geospatial system integration, not build a commercial product
- Stack: 
  • Frontend: React (not started yet) + Leaflet + OpenStreetMap
  • Backend: ASP.NET Core Web API (REST)
  • Database: PostgreSQL + PostGIS (running in local Docker)
  • Infra: Local development only; no cloud deployment required for thesis


# Project Overview

> *Bachelor Thesis | 4-Month Scope | Junior-Level Implementation*
